...
{
	constexpr auto numbers = stoba::make<"55 8B EC 83 E4 F8 8B 4D 08 BA ? ? ? ? E8 ? ? ? ? 85 C0 75 12 ?">();
...
int main()
{
	constexpr static auto result = combine_two<3>();
	static_assert( result == 88 );
	printf( "%d\n", result );

	constexpr auto numbers = stoba::make<"55 8B EC 83 E4 F8 8B 4D 08 BA ? ? ? ? E8 ? ? ? ? 85 C0 75 12 ?">();
	for( const auto &hex : numbers )
	{
		printf( "%x ", hex );
	}
}
